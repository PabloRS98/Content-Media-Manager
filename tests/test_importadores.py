...
class TestImportadorDeLibros:
    def test_importa_un_export_de_goodreads(self, db):
        csv = (
    ...
    def test_importa_un_export_de_storygraph(self, db):
        csv = (
    ...
    def test_no_duplica_dentro_del_mismo_fichero(self, db):
        csv = (
    ...
    def test_no_duplica_entre_importaciones(self, db):
        csv = "Title,Author,Exclusive Shelf\nDune,Frank Herbert,read\n"
        import_books_csv(db, csv)
        res = import_books_csv(db, csv)

        assert res["creados"] == 0
        assert res["duplicados"] == 1

    def test_una_fila_sin_titulo_se_omite(self, db):
        csv = "Title,Author\n,Sin titulo\nDune,Frank Herbert\n"
        res = import_books_csv(db, csv)
    ...
    def test_el_mismo_titulo_de_otro_autor_no_es_duplicado(self, db):
        csv = (
            "Title,Author\n"
            "Ulises,James Joyce\n"
            "Ulises,Otro Autor\n"
...
class TestImportadorDeJuegos:
    def test_importa_un_csv_generico(self, db):
        csv = (
    ...
    def test_traduce_los_estados_de_backloggd(self, db):
        csv = (
            "Name,Status\n"
            "Uno,backlog\n"
            "Dos,abandoned\n"
    ...
    def test_unas_horas_ilegibles_no_rompen_la_importacion(self, db):
