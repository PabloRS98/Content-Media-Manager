...
def serie_con_temporadas(db, usuario, temporadas: dict[int, date | None], titulo="Serie"):
...
class TestQueTemporadasSePiden:
    ...
    def test_se_pide_la_temporada_con_episodios_sin_emitir(self, usuario, db):
        item = serie_con_temporadas(db, usuario, {
    ...
    def test_se_pide_la_temporada_emitida_hace_poco(self, usuario, db):
    ...
    def test_se_pide_una_temporada_que_no_tenemos(self, usuario, db):
        item = serie_con_temporadas(db, usuario, {1: HOY - timedelta(days=900)})
        assert temporadas_que_pueden_cambiar(item, [1, 2]) == [2]

    def test_se_pide_una_temporada_sin_fechas_conocidas(self, usuario, db):
        """Sin `air_date` no se puede afirmar que esté cerrada."""
        item = serie_con_temporadas(db, usuario, {1: None})
        assert temporadas_que_pueden_cambiar(item, [1]) == [1]

    def test_una_serie_sin_episodios_pide_todo(self, usuario, db):
        item = serie_con_temporadas(db, usuario, {})
        assert temporadas_que_pueden_cambiar(item, [1, 2, 3]) == [1, 2, 3]

    def test_sin_temporadas_remotas_no_se_pide_nada(self, usuario, db):
        item = serie_con_temporadas(db, usuario, {1: HOY})
...
class TestRefrescoCompleto:
    ...
    def test_una_serie_terminada_no_consume_peticiones_de_temporadas_viejas(
        self, db, usuario, tmdb_falso, fabrica_de_sesiones
    ):
        serie_con_temporadas(db, usuario, {
            1: HOY - timedelta(days=1500),
    ...
    def test_las_series_se_refrescan_en_paralelo(self, usuario, db, tmdb_falso, fabrica_de_sesiones):
        ...
        for n in range(8):
            serie_con_temporadas(db, usuario, viejas, titulo="Serie %02d" % n)
    ...
    def test_un_fallo_en_una_serie_no_pierde_las_demas(
        self, db, usuario, tmdb_falso, fabrica_de_sesiones, monkeypatch
    ):
    ...
    def test_sin_clave_de_tmdb_no_hace_nada(self, usuario, db, monkeypatch, fabrica_de_sesiones):
        monkeypatch.setattr(scheduler.settings, "tmdb_api_key", "")
        serie_con_temporadas(db, usuario, {1: HOY})
        scheduler.refresh_following_episodes(db, session_factory=fabrica_de_sesiones)
