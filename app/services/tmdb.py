...
def _genre_map(kind: str) -> dict[int, str]:
    ...
    try:
        ...
        logger.warning("Fallo al obtener géneros TMDB (%s): %s", kind, describe(exc))
        return {}
...
def _search(endpoint: str, media_label: str, query: str, limit: int, year: int | None = None) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Fallo al buscar %s: %s", media_label, describe(exc))
        return []
...
def find_by_imdb_id(imdb_id: str, media_type: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("Fallo al buscar por IMDb ID '%s' en TMDB: %s", imdb_id, describe(exc))
        return None
...
def get_movie_details(tmdb_id: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("Fallo al obtener detalles de película TMDB %s: %s", tmdb_id, describe(exc))
        return None
...
def get_tv_details(tmdb_id: str) -> dict | None:
    ...
    try:
        ...
        logger.warning("Fallo al obtener detalles de serie TMDB %s: %s", tmdb_id, describe(exc))
        return None
...
def fetch_tv_episodes(tmdb_id: str, seasons: list[int]) -> list[dict]:
    ...
    for sn in seasons:
        try:
            ...
            logger.warning("Fallo al obtener episodios TMDB %s temporada %s: %s", tmdb_id, sn, describe(exc))
            continue
    return out
